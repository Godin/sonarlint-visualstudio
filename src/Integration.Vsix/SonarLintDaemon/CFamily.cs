...
namespace SonarLint.VisualStudio.Integration.Vsix
{
    ...
    {
        public static string TryGetConfig(string absoluteFilePath)
        {
            try
            {
                return FileConfig.TryGet(absoluteFilePath).ToJson();
            }
            catch (Exception e)
            {
                // TODO use VsShellUtils.WriteToSonarLintOutputPane()
        ...
        {
            public static FileConfig TryGet(string absoluteFilePath)
            {
                // TODO maybe IssueTracker can store projectItem?
                EnvDTE.DTE dte = ServiceProvider.GlobalProvider.GetService<EnvDTE.DTE>();
                EnvDTE.ProjectItem projectItem = dte.Solution.FindProjectItem(absoluteFilePath);
            ...
            public Capture[] ToCaptures()
            {
                ...
                Add(c.Cmd, ConvertCompileAs(CompileAs));
            ...
            internal static string ConvertCompileAs(string value)
            {
                switch (value)
                {
                    default:
